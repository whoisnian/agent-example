...
- **Dependencies**: add `deepagents` (and its transitive LangChain deps) to `worker/pyproject.toml`; add the provider chat-model package behind the `ModelFactory` (e.g. `langchain-anthropic`). No new infra. Tests add no network dependency thanks to the fake model.
