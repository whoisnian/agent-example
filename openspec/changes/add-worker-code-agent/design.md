...
`agents/model.py` defines `ModelFactory.get(model_key: str) -> BaseChatModel`. Production factory maps `model_key` → provider model (e.g. `langchain-anthropic` `ChatAnthropic`) using config + env API key. Agents never import a provider SDK; they call `ctx`-independent `self._models.get(self.model_key)`. Tests pass a `FakeModelFactory` returning a **scripted** `FakeChatModel` whose responses drive a deterministic plan→execute→critic transcript.
