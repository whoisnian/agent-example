...
### D5 · `:root`/`.dark` 约定翻转归 C
B 把 light 值 authored-inactive 在 `.light`、dark 值留在 `:root`（默认不变）。C 在接通切换时完成标准化翻转：把 `.light` 的值搬进 `:root`（light）、dark 值搬进 `.dark`，`<html>` 按偏好挂/去 `.dark`。这是默认主题行为翻转的唯一落点，对应本 change 对 `web-design-system` 的 MODIFIED delta（A 定义"默认场景"、B 不碰、C 改写）。
- **理由**：把所有 `:root`/`.dark` 语义变化集中在"主题行为"这个 change，B 保持纯值层零结构改动，三方不争用同一 spec 块。
