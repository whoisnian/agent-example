<!-- C MUST be applied/archived strictly after A (and B). This MODIFIED block restates
     `CSS-Variable Theme Tokens` (as it stands after A) and changes ONLY the default-theme
     behavior: the "Default theme resolves without a class toggle" scenario → preference-driven
     FOUC-safe boot. The `:root`=light / `.dark`=dark convention is already established by B
     (the neutral-minimal redesign), so C performs NO value/convention reshuffle — only the
     toggle mechanism. -->
