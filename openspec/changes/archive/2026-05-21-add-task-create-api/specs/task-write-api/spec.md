...
- **WHEN** a client `POST`s `/api/v1/tasks` with a valid body (`title`, `task_type`, `prompt`)
...
- **WHEN** `POST /api/v1/tasks` is called with empty `title`
