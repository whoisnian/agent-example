...
- `api.ts`: `getVersionArtifacts(versionId, signal)` → `/versions/{id}/artifacts` with **`toastOnError:false`** (`ArtifactList` renders its own inline error/empty state per expanded row, so the inline state is the single error surface — mirrors `getMyCost`+`useMyCostQuery`, suppressing both toast layers); `getArtifactPresign(artifactId, signal)` → `/artifacts/{id}/presign` with **`toastOnError:false`** (the calling component owns the error UX — see D4).
