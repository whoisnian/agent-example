...
### Requirement: Execution Dispatcher (Placeholder)

The runtime SHALL include an `ExecutionDispatcher` that, given a parsed `TaskExecuteMessage` and prepared `RunContext`, looks up the appropriate agent by `task_type` and invokes it. In the MVP scaffold the dispatcher MUST always raise `AgentNotImplementedError` (translated to a final `task.events` `error` event with code `unimplemented` and a `nack(requeue=false)` so the message goes to DLX) — no real agent exists at this stage.

This requirement exists so that the end-to-end message → consume → context → publish → dlx path is testable without any real LLM call.

#### Scenario: Unknown task_type produces DLX
- **WHEN** a `task.execute` message with `task_type="code-gen"` is consumed in the scaffold
