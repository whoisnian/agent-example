# task-conversation-history Specification

## Purpose

同一 task 的对话历史契约：历史的来源（task_versions 父链）、组装规则（顺序、深度与字节上限、缺摘要降级）、execute payload 中 history 字段的形态与定格语义，以及 Worker 注入模型输入的最低要求。Established by archiving change `refactor-task-conversation-continuity`.
