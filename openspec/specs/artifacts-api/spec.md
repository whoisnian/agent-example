...
HTTP read surface for task artifacts: an owner-scoped per-version metadata list and a short-lived, single-object presigned download URL. The API never proxies object bytes — it lists metadata (never leaking `oss_key`) and mints presigned GET URLs that the browser uses to download directly from the S3-compatible OSS. Established by archiving change `add-artifacts-api`.
