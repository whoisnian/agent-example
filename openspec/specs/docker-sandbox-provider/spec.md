...
## Purpose
Provides `DockerSandboxProvider` and `DockerSandbox` classes that back deepagents agents with a Docker container, routing all filesystem tools and shell commands through the container via `execute()`.
