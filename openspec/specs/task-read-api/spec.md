...
The API SHALL expose `GET /api/v1/versions/{version_id}` that returns one version reachable through a task the caller owns. The response MUST be HTTP `200` with `data = {version, runs, cost}`, where `version` is the full version row — including `prompt` and `params` (rendered as raw JSON) and a concrete boolean `is_active` — `runs` is the array of that version's `task_runs` ordered by `attempt_no` ascending (each exposing `{id, attempt_no, status, started_at, ended_at, last_heartbeat, error}`, where `error` is raw JSON and MUST be JSON `null` when the column is `NULL`), and `cost` is the version-level cost summary. `runs` MUST always be an array, never `null`.
