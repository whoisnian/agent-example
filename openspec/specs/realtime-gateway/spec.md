...
The server half of the realtime channel (`docs/ARCHITECTURE.md §5.2 / §6.2`): a `GET /api/v1/ws` WebSocket endpoint that closes the "submit → execute → observe live → retrieve" loop. It handles connection/auth lifecycle, the subscribe/unsubscribe/ping protocol, owner-scoped topic subscriptions, per-instance event fan-out from the `task.events` exchange, backpressure/slow-client eviction, and graceful shutdown — feeding the already-built `web-realtime-client` so the UI observes task progress live instead of polling. Established by archiving change `add-realtime-gateway`.
