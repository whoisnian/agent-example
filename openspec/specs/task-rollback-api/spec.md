# task-rollback-api Specification

## Purpose

The API's rollback capability: `POST /api/v1/tasks/{task_id}/rollback` with two modes — `branch` (create + execute a new version parented on a historical version, reusing the iterate mutex/outbox/execute machinery) and `switch` (repoint `tasks.current_version` at a historical terminal version only, no execution, no `tasks.status` write). Both modes are owner-scoped and require the task to have no active version. Established by archiving change `add-task-rollback-api`.
