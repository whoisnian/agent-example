# api-user-store Specification

## Purpose

The API's persistent identity store: the `tenants` and `users` schema, bcrypt password hashing with case-insensitive email uniqueness, the database-backed credential verification consumed by `POST /api/v1/auth/login`, and the idempotent boot-time dev-user seed that bootstraps the store from configuration. It replaces the former in-memory dev-principal comparison so two different users resolve to two different owners. Established by archiving change `add-api-user-store`.
