...
### Requirement: Declare CustomContext as context_schema and pass start_time to astream
`main()` SHALL pass `context_schema=CustomContext` to `create_deep_agent()` and supply `context=CustomContext(start_time=start_time)` in the keyword arguments to `agent.astream()` so that all middleware in the pipeline can access the typed context.

#### Scenario: start_time propagated to middleware
- **WHEN** `agent.astream()` is called with `context=CustomContext(start_time=start_time)`
